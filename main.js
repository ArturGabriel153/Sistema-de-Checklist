...
/* ── 4. Partículas flutuantes no hero (poucas para não travar) ─ */
(function spawnParticles() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    p.style.cssText = `
      ...
      border-radius:50%;
      background:rgba(27,58,138,${Math.random() * 0.1 + 0.04});
...
/* Flutuação suave do mockup — começa só após a entrada terminar */
tl.call(() => {
  gsap.to('#heroMockup', {
    y: -10, duration: 3.5,
...
/*  Todos usam fromTo() para garantir que o estado final é opacity:1
    mesmo que o CSS tenha definido opacity:0 no elemento.           */
  ...
  onEnter: () => {
    ...
    document.querySelectorAll('.arch-layer').forEach((layer, i) => {
      gsap.fromTo(layer.querySelectorAll('.arch-box'),
        ...
        { opacity: 1, scale: 1, stagger: 0.08, duration: 0.42, ease: 'back.out(1.4)', delay: 0.28 + i * 0.18 });
    });
  },
});

/* ── 12. Perfis — listas de permissão ────────────────────────── */
document.querySelectorAll('.perfil-card').forEach(card => {
  ScrollTrigger.create({
    ...
    once: true,
    onEnter: () => {
      gsap.fromTo(card.querySelectorAll('.perm-list li'),
        { opacity: 0, x: -14 },
        { opacity: 1, x: 0, stagger: 0.045, duration: 0.32, ease: 'power2.out', delay: 0.15 });
    },
  });
});

/* ── 13. Stack gsap-row: glow de destaque ────────────────────── */
...
if (gsapRow) {
  ScrollTrigger.create({
    ...
    onEnter: () => {
      gsap.fromTo(gsapRow,
        { boxShadow: '0 0 0px rgba(27,58,138,0)' },
        { boxShadow: '0 0 24px rgba(27,58,138,0.22)', duration: 0.9,
          ease: 'power2.out', yoyo: true, repeat: 1 });
    },
  });
}

/* ── 14. Pulse no notif-dot do mockup ────────────────────────── */
gsap.to('.notif-dot', {
...
function updateActiveNav() {
  ...
  navLinks.forEach(a => {
    ...
    a.style.color      = active ? 'var(--azul)' : '';
    a.style.background = active ? 'var(--azul-light)' : '';
